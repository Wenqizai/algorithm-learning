...
public class P02MaxSubArray {
    public static void main(String[] args) {
        ...
        System.out.println(divide(arr, 0, arr.length - 1, Integer.MIN_VALUE));
    }
     ...
     * 分治
     */
    private static int divide(int[] nums, int left, int right, int maxSum) {
        if (left == right) {
//            if (nums[left] > maxSum) {
//                maxSum = nums[left];
//            }
        ...
        divide(nums, left, mid, maxSum);
        divide(nums, mid + 1, right, maxSum);

        int leftMaxSum = maxSubArrayNum(nums, left, mid);
        int rightMaxSum = maxSubArrayNum(nums, mid + 1, right);
        int crossMaxNum = maxCrossSubArrayNum(nums, left, mid, right);

        if (leftMaxSum >= rightMaxSum && leftMaxSum >= crossMaxNum) {
            maxSum = leftMaxSum;
        } else if (rightMaxSum >= leftMaxSum && rightMaxSum >= crossMaxNum) {
            maxSum = rightMaxSum;
        } else {
            maxSum = crossMaxNum;
        }
    ...
    private static int maxSubArrayNum(int[] nums, int left, int right) {
        int sum = 0;
        while (left <= right) {
            sum += nums[left];
            left++;
    ...
    private static int maxCrossSubArrayNum(int[] nums, int left, int mid, int right) {
        int rightMid = mid + 1;
        int leftSum = 0;
        ...
        while (rightMid <= right) {
            rightSum += nums[rightMid];
            if (rightSum > leftMaxSum) {
    ...
    /**
     * 滑动窗口算法, 时间超出限制, 存在大量重复计算
     */
    private static void slide(int[] nums) {
        int maxSum = Integer.MIN_VALUE;
        int maxIndex = nums.length - 1;
        while (maxIndex >= 0) {
            maxSum = doSlide(nums, 0, maxIndex, maxSum);
            maxIndex--;
        }
    }

    private static int doSlide(int[] arr, int left, int right, int maxSum) {
        if (right >= arr.length) {
            return maxSum;
        }
        ...
        if (sum > maxSum) {
            maxSum = sum;
            System.out.printf("found a maxSum: %s, left: %s, right: %s%n", maxSum, left, right);
        }
